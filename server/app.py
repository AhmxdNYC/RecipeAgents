...
# Ensure the graph module can be imported
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from graph.graph import process_user_input  # Ensure this path is correct

app = Flask(__name__)
CORS(app)

def is_valid_input(text):
    # Simple validation: input should be more than 8 characters long
    return len(text) > 8
...
def process_data():
    ...
    print("Received input:", input_text)  # Debug print
    ...
    try:
        ...
        print("Processed result:", processed_result)  # Debug print

        return jsonify(processed_result)
    except Exception as e:
        print(f"Error processing input: {e}")  # Debug print
        return jsonify({"error": "An error occurred during processing"}), 500

@app.route('/api/processed', methods=['POST'])
def receive_processed_data():
    data = request.json
    print("Received processed data:", data)  # Debug print
